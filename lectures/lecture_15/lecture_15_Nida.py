...
def main():
    ...
    i = 0
    for club in premier_league:  # for every club in the outer loop
        for detail in club_details:   # have to search through every list in that details list and see if you can match names up
            name = get_name(detail)
            if name.lower().find(get_name(club).lower()) > -1: # find method returns either a postive index where the string is found or it returns -1 if nothing found
                premier_league[i][0] = name
                break
        i += 1
    ...
    for club in premier_league:
        for detail in club_details:
            ...
            if club[-1].lower() == stadium[0].lower() and stadium[1] >= 60000:
                ...
                break #don't have to but it's more efficient
    ...
    standings = None # TODO Implement

    # Implement loop (after implementing function)
