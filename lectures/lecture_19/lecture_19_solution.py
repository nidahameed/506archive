...
def get_scholars(publications):
    ...
    for publication in publications:
        authors = publication['Authors'].split('; ') # note semi-colon delimiter
        for author in authors:
            name = author.split(', ')
            if name not in scholars:
...
def has_umsi_faculty_coauthor(publication, names, ignore=None):
    ...
    for author in authors:
        ...
        for name in names:
            if match_name(author_name, name, ignore):
                return True
    return False
...
def get_most_cited_pubs(publications, headers):
    ...
    for publication in publications:
        count = int(publication[headers.index('Total Citations')]) # convert to int
        if count > prev_count:
    ...
    for publication in publications:
        for key in publication.keys():
            if key == 'Publication Year':
                year = publication[key]
                if year in pub_counts.keys():
                    pub_counts[year] += 1
                else:
                    pub_counts[year] = 1 # new key-value pair
...
def main():
    ...
    most_cited = get_most_cited_pubs(publications[1:], headers) # returns list
    ...
    pub_counts = get_pub_counts_by_year(publications)
    ...
    # for scholar in sorted(scholars, key=lambda x: (x[0], x[1]), reverse=True):
        last_name = scholar[0].lower()
        initial = scholar[1][0].lower() # heroic assumption
        name = [last_name, initial]
        if name not in filter:
            filter.append(name) # add to filter
            scholars_filtered.append(scholar) # names for file
    ...
    for member in umsi_faculty:
        name = member.split(', ')
        for scholar in scholars_filtered:
            if match_name(name, scholar, 'resnick'):
               umsi_coauthors.append(name) # full name
               break
    ...
    for publication in publications:
        if has_umsi_faculty_coauthor(publication, umsi_coauthors, ignore='resnick'):
