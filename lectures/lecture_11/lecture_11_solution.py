...
def get_oldest_cereal(cereals):
    ...
    for cereal in cereals:
        ...
        if current_year < prev_year:
            ...
            prev_year = get_cereal_year(cereal) # reset
...
def has_ingredient(cereal, ingredient):
    if get_cereal_ingredients(cereal).lower().find(ingredient.lower()) > -1:
    # if cereal[-1].lower().find(ingredient.lower()) > -1:
        return True
    else:
        return False
...
def search_ingredients(cereals, ingredient):
    results = []
    for cereal in cereals:
        if has_ingredient(cereal, ingredient):
            name = format_cereal_name(cereal)
            results.append(name)
    return results
