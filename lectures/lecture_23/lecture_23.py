...
class Person:
    ...
    def jsonable(self):
        ...
        return {
                ...
                'homeworld': self.homeworld.jsonable() #have to call .jsonable() method so that the entire dict literal is itself composed of dicts
            }
        # json module doens't know how to serialize/ecode custom object we've created so we use this kind of solution
        # make sure to call .jsonable() -- won't work without it
...
class Starship:
    ...
    def update_crew_members(self, crew):
        ...
        if self.crew_members:
            self.crew_members.update(crew) #Crew instance does the updating
...
def main():
    ...
    wookiee_m_falcon = read_json(filepath) #decoding json doc into a dict; this is a dict
    ...
    #COMBINING M_FALCON DICTIONARIES
    swapi_m_falcon.update(wookiee_m_falcon)
    ...
    # TODO UNCOMMENT
    m_falcon = Starship(swapi_m_falcon['url'], swapi_m_falcon['name']) #instantiated a Starship
    m_falcon.starship_class = swapi_m_falcon['starship_class'] #assigning the relevant dict values to each instance variable
    ...
    # TODO INSTANTIATE INSTANCE OF CREW
    crew = Crew({'pilot' : solo, 'co_pilot': chewie})
